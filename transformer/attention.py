...
class AttentionStack(nn.Module):
    def __init__(self,  n: int, D_embed: int, N_heads: int, model=AttentionBlock):
        super(AttentionStack, self).__init__()
        self.stack = nn.ModuleList([model(D_embed, N_heads) for _ in range(n)])
...
class CrossAttentionStack(nn.Module):
    def __init__(self, n: int, D_embed: int, N_heads: int, dropout=0.1, model = CrossAttentionBlock):
        """
        ...
        self.stack = nn.ModuleList([model(D_embed, N_heads, dropout=dropout) for _ in range(n)])
    ...
    def __init__(self, n: int, D_embed: int, N_heads: int, dropout=0.1):
        """
        CrossAttentionStack: Stacks multiple Cross-Attention Blocks
        Args:
        - n: Number of Cross-Attention Blocks
        ...
        """
        super(CrossAttentionStack, self).__init__()
        self.stack = nn.ModuleList([CrossAttentionBlock(D_embed, N_heads, dropout=dropout) for _ in range(n)])
    ...
    def __init__(self, n: int, D_embed: int, N_heads: int):
        super(CrossAttentionStack, self).__init__()
        self.stack = nn.ModuleList([CrossAttentionBlock(D_embed, N_heads) for _ in range(n)])
...
if __name__ == "__main__":
    ...
    self_input = torch.randn(B, N_seq_self, D_embed)  # Input for self-attention

    # Initialize Self-Attention Block
    self_attention_block = AttentionBlock(D_embed=D_embed, N_heads=N_heads)
    ...
    decoder_input = torch.randn(B, N_seq_decoder, D_embed)  # Query from decoder
    encoder_output = torch.randn(B, N_seq_encoder, D_embed)  # Key/Value from encoder

    # Initialize Cross-Attention Block
    cross_attention_block = CrossAttentionBlock(D_embed=D_embed, N_heads=N_heads)
    ...
    # Dummy Inputs
    decoder_input = torch.randn(B, N_seq_decoder, D_embed)  # (B, N_seq_decoder, D_embed)
    encoder_output = torch.randn(B, N_seq_encoder, D_embed)  # (B, N_seq_encoder, D_embed)
    
    
    # Cross-Attention Stack
    print("-" * 20 + " Cross-Attention Stack Example " + "-" * 20)
    
    cross_attention_stack = CrossAttentionStack(n=n_blocks, D_embed=D_embed, N_heads=N_heads)
